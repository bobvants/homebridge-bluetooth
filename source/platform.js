...
BluetoothPlatform.prototype.scan = function () {
  let b = false;
  for (var id of Object.keys(this.bluetoothAccessories)) {
    var bluetoothAccessory = this.bluetoothAccessories[id];
    if (!bluetoothAccessory.isConnected) {
      this.log.debug(bluetoothAccessory.name, " is not connected |||")
      b = true;
      break;
    }
  }
...
BluetoothPlatform.prototype.discover = function (nobleAccessory) {
  ...
  for (var id of Object.keys(this.bluetoothAccessories)) {
    var bluetoothAccessory = this.bluetoothAccessories[id];
    if (trimAddress(bluetoothAccessory.address) == accessoryAddress) {
      isMyDevice = true;
      break;
    }
  }
  ...
  if (isMyDevice) {
    ...
    nobleAccessory.connect(function (error) {
      self.connect(error, nobleAccessory);
    }.bind(self));
  }
  else {
...
BluetoothPlatform.prototype.connect = function (error, nobleAccessory) {
  ...
  var peripheralId = trimUUID(nobleAccessory.address);
  ...
  for (var id of Object.keys(this.bluetoothAccessories)) {
    ...
    if (peripheralId != trimAddress(bluetoothAccessory.address)) {
      continue
    }

    bluetoothAccessory.isConnected = true;
...
BluetoothPlatform.prototype.disconnect = function ( nobleAccessory ) {
  ...
  for (var id of Object.keys(this.bluetoothAccessories)) {
    ...
    bluetoothAccessory.isConnected = false;
...
BluetoothPlatform.prototype.discoverServices = function (error, nobleServices) {
  ...
  for (var nobleService of nobleServices) {
    ...
    this.log.debug("XXXXXXXX Discovered service | ", nobleService._peripheralId );

    var peripheralId = trimUUID(nobleService._peripheralId);
    ...
    for (var id of Object.keys(this.bluetoothAccessories)) {
      ...
      for (var serviceID of Object.keys(bluetoothAccessory.bluetoothServices)) {
        ...
        if (trimUUID(bluetoothService.UUID) == serviceUUID) {
          ...
          for (var characteristicID of Object.keys(bluetoothService.bluetoothCharacteristics)) {
            var bluetoothCharacteristic = bluetoothService.bluetoothCharacteristics[characteristicID];

            if (!bluetoothCharacteristic.UUID && bluetoothCharacteristic.constant) {
              var homebridgeCharacteristic = homebridgeService.getCharacteristic(bluetoothCharacteristic.class);
              bluetoothCharacteristic.connect(null, homebridgeCharacteristic);
            }
          }
        }
      }
...
BluetoothPlatform.prototype.discoverCharacteristics = function (error, nobleCharacteristics) {
  ...
  for (var nobleCharacteristic of nobleCharacteristics) {
    ...
    var peripheralId = trimUUID(nobleCharacteristic._peripheralId);
